...
sub afterRenameHandler {
  ...
  _writeDebug("called afterRenameHandler(oldWeb=$oldWeb, oldTopic=$oldTopic, newWeb=$newWeb, newTopic=$newTopic)") if TRACE;
...
sub beforeSaveHandler {
  ...
  my $topicTitleField = $meta->getPreference("TOPICTITLE_FIELD") || "TopicTitle";
  ...
  if (defined $topicTitleHash) {
    _writeDebug("found old TopicTitle in preference settings: $topicTitleHash->{value}") if TRACE;
    if ($topicTitle) {
  ...
  _writeDebug("no TopicTitle stored anywhere. creating a new preference setting")
    if TRACE;
...
sub getTopicTitle {
  ...
  if (defined $topicTitle) {
    _writeDebug("... found topicTitle for $key in cache (web='$web', topic='$topic'") if TRACE;
    return $topicTitle;
  }
  ...
  $topicTitle = $meta->get('FIELD', $topicTitleField . $lang);
  $topicTitle = $meta->get('FIELD', $topicTitleField) unless $topicTitle;
  ...
  unless ($topicTitle) {
    ...
    $topicTitle = $meta->getPreference('TOPICTITLE') unless $topicTitle;
  ...
  unless ($topicTitle) {
    ...
    $topicTitle = $meta->get('PREFERENCE', 'TOPICTITLE') unless $topicTitle;
    $topicTitle = $topicTitle->{value} if $topicTitle;
